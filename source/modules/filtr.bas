...
Function pobierzKryteria(f As Form) As String
    ...
    For Each c In f.headerForm.Controls     'Debug.Print c.Name
        ...
        ElseIf c.Name Like "f[A-Z]*" And TypeName(c) = "ComboBox" Then
            Call dodajTekst(k, c)
        End If
    Next c
    pobierzKryteria = k
...
Function getDodatkowyFiltr(f As Form) As String
    On Error GoTo blad
        getDodatkowyFiltr = f.dodatkowyFiltr
    Exit Function
blad:
    getDodatkowyFiltr = ""
End Function

Sub setDodatkowyFiltr(f As Form, dodatkowyFiltr As String)
    On Error GoTo blad
        f.dodatkowyFiltr = dodatkowyFiltr
    Exit Sub
blad:
    Call MsgBox("Próbujesz ustawiæ dodatkowy filtr na formularzu " & f.Name)
End Sub

Sub filtrujFormularz(f As Form) 'Optional dodatkowyFiltr As String = ""
    Dim dodatkowyFiltr As String
    dodatkowyFiltr = getDodatkowyFiltr(f)
    Dim k As String
    k = pobierzKryteria(f)
